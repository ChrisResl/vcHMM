...
def viterbi(emission_matrix, transmission_matrix):
    ...
    Important:  if in Ri is no list, but a -1 -> skip this part.
                if skip part because of -1: use initialprob for first not -1 Ri 
    """
    for i in range(len(emission_matrix)):
        ...
        if i == 0 or emission_matrix[i] != -1 and emission_matrix[i-1] == -1:
            ...
            for y in range(30):
                for z in range(30):
                    if delta_matrix[y][z] != "NaN":
                        true_temp = float(transmission_matrix[y][z])*float(delta_matrix[y][z])
            ...
            for y in range(30):
                if "nan" != max_list:
                    emmi_list.append(float(max_list[y])+float(emission_matrix[i][y]))
            max_list = []
